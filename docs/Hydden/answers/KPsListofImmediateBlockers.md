# **`Keerthana's Immediate List of Blockers`**:
---

# Table of Contents 
> - #### ( *`SEV - 1`* | *`SEV - 2`* | *`SEV - 2.5`* | *`SEV - 3`* )
> - ***Details and Explanation after Table-of-Contents***.
> - *See the **`Ultimate Production Checklist Doc`** as my (KP's) support document to feed as pre-context to AI code-building agents.*
> ---
...
> #### a. **`consumer.go`**
> 
> > 1. **Build blocker: unused `crypto/subtle` import**: *preflight `go build` / `go test ./...` failure*
> > 
> > 2. **Plaintext password stored in projection**: *credential exposure*
> > 
> > 3. **Raw password written to immutable event log**: *permanent secret leakage*
> > 
> > 4. **Reset token path lacks secure token lifecycle**: *timing, replay, expiry, single-use, scope, & attempt-limit gaps*
> > 
> > 5. **Ack after failed processing**: *event loss*
> > 
> > 6. **No idempotency/dedupe**: *duplicate delivery corrupts state*
> > 
> > 7. **No per-account serialization**: *out-of-order projection updates*
> > 
> > 8. **No optimistic concurrency**: *stale writes overwrite newer state*
> > 
> > 9. **Ignored JSON decode errors**: *malformed events mutate state*
> > 
> > 10. **Unknown event types advance version**: *projection drift*
> > 
...
> > 
> > 2. **Oversimplified impossible travel**: *weak fraud feature*
> > 
> > 3. **Implicit feature ordering**: *schema fragility / training-serving skew risk*
> > 
> > 4. **Missing-label crash risk**: *brittle training*
> > 
> > 5. **No random seed/config**: *non-reproducible model*
> > 
> > 6. **No model governance**: *unsafe ML release*
> > 
> > 7. **No scoring schema validation**: *live crash/mis-score risk*
> > 
> > 8. **Empty-history default gap**: *cold-start / feature-store / future scoring fragility*
> > 
...
> #### a. **`consumer.go`**
> 
> > 1. **Standard logger only**: *weak structured logging*
> > 
> > 2. **Unsafe concurrency implementation despite correct intent**: *scalability bug*
> > 
> > 3. **Ignored JSON marshal errors**: *low-likelihood event-construction error handling gap*
> 
> #### b. **`risk_scorer.py`**
> 
...
| ---: | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1. | ***Ln***: 10<br><br>**Build blocker: unused `crypto/subtle` import**                                 | `go build` / `go test ./...` fails before runtime review.                                    | Remove the import or actually use `subtle.ConstantTimeCompare`.                                                                           | This is more immediate than runtime findings because the submitted Go file should not pass preflight build.<br><br>**`Refs`**: [*go.dev/unused-imports*](https://go.dev/doc/faq#unused_variables_and_imports), [*pkg.go.dev/crypto/subtle*](https://pkg.go.dev/crypto/subtle)                                                                  |
|   2. | ***Ln***: 29-36, 89-92<br><br>**Plaintext password stored in projection via `PasswordPlain`**        | Direct credential exposure in an identity system.                                            | Remove `PasswordPlain`.<br><br>Store only approved password-hash metadata.                                                                | Test plaintext never appears in state, logs, traces, or events.<br><br>**`Refs`**: [*cheatsheetseries.owasp.org/password-storage*](https://cheatsheetseries.owasp.org/cheatsheets/Password_Storage_Cheat_Sheet.html), [*cwe.mitre.org/CWE-256*](https://cwe.mitre.org/data/definitions/256.html)                                               |
|   3. | ***Ln***: 101-109<br><br>**Raw password written into immutable event payload as `NewPassword`**      | Permanently pollutes append-only audit log with credentials.                                 | Never place passwords, tokens, or secrets in events.<br><br>Append safe metadata only.                                                    | Add event-schema forbidden-field tests.<br><br>Add secret scanning for generated event fixtures.<br><br>**`Refs`**: [*cheatsheetseries.owasp.org/logging*](https://cheatsheetseries.owasp.org/cheatsheets/Logging_Cheat_Sheet.html)                                                                                                            |
|   4. | ***Ln***: 101-103<br><br>**Reset token path lacks secure token lifecycle**                           | Timing/replay risk; expiry, single-use, scope, account binding, & attempt-limit gaps remain. | Store hashed reset tokens.<br><br>Use constant-time compare.<br><br>Enforce expiry, single-use, scope, account binding, & attempt limits. | `crypto/subtle` is imported but unused.<br><br>**`Tests`**: expired token, reused token, wrong account, attempt throttling.<br><br>**`Refs`**: [*pkg.go.dev/crypto/subtle*](https://pkg.go.dev/crypto/subtle), [*cheatsheetseries.owasp.org/forgot-password*](https://cheatsheetseries.owasp.org/cheatsheets/Forgot_Password_Cheat_Sheet.html) |
|   5. | ***Ln***: 62-65<br><br>**Broker `ack(e)` runs even when `apply` fails**                              | Failed events can be permanently lost.                                                       | Ack only after durable success.<br><br>Retry, nack, or DLQ on failure.                                                                    | **`Tests`**: `LoadState` failure, `SaveState` failure, validation failure must not ack success.                                                                                                                                                                                                                                                |
|   6. | ***Ln***: 56-66, 71-96<br><br>**No idempotency/dedupe despite at-least-once delivery**               | Duplicate delivery corrupts counters, IP history, password state, & versions.                | Track processed `Event.ID` atomically or enforce version-idempotency.                                                                     | **`Tests`**: duplicate `LoginFailed` does not increment twice.<br><br>Duplicate `LoginSucceeded` does not append twice.                                                                                                                                                                                                                        |
|   7. | ***Ln***: 56-66, 71-96<br><br>**No per-account serialization**                                       | Same-account events can apply out of order.                                                  | Partition/serialize workers by account/aggregate key.                                                                                     | Add race tests & same-account ordering tests.<br><br>**`Refs`**: [*go.dev/race-detector*](https://go.dev/doc/articles/race_detector)                                                                                                                                                                                                           |
|   8. | ***Ln***: 39-43, 94-96<br><br>**`SaveState` has no optimistic concurrency / expected-version check** | Stale projection writes can overwrite newer account state.                                   | Change API to `SaveState(ctx, state, expectedVersion)` with CAS/transaction semantics.                                                    | **`Tests`**: stale write, gap version, duplicate version, concurrent same-account update.                                                                                                                                                                                                                                                      |
|   9. | ***Ln***: 82-91<br><br>**`json.Unmarshal` errors ignored**                                           | Malformed payloads silently mutate state with zero values.                                   | Check all decode errors.<br><br>Validate schema, required fields, field types, & payload sizes.                                           | **`Tests`**: malformed JSON, missing fields, wrong types, empty IP/password.<br><br>**`Refs`**: [*pkg.go.dev/encoding/json*](https://pkg.go.dev/encoding/json)                                                                                                                                                                                 |
|  10. | ***Ln***: 80-92, 94-96<br><br>**Unknown event types silently advance projection version**            | Projection can skip required state changes while moving forward.                             | Add `default` case.<br><br>Reject/quarantine unknown types.<br><br>Do not save as success.                                                | **`Tests`**: unknown type does not advance version.<br><br>DLQ/quarantine path emits telemetry.                                                                                                                                                                                                                                                |
...
|   2. | ***Ln***: 39-42<br><br>**`detect_impossible_travel` is oversimplified**              | Country-count heuristic is not actual impossible travel.                                                                           | Use geo distance/time deltas & validated feature definition.                                           | Feature-review test & doc.                                                                                                                                                                                                                                                               |
|   3. | ***Ln***: 61, 76<br><br>**Implicit feature ordering via `dict.values()`**            | Schema fragility / training-serving skew risk if feature construction changes.                                                     | Define `FEATURE_COLUMNS`; vectorize by explicit schema.                                                | Schema compatibility test.<br><br>**`Refs`**: [*docs.python.org/dict*](https://docs.python.org/3/library/stdtypes.html#mapping-types-dict)                                                                                                                                               |
|   4. | ***Ln***: 62<br><br>**Label lookup can crash on missing label**                      | Training can fail or create biased workaround risk.                                                                                | Validate label coverage; define unlabeled-account policy.                                              | Tests for missing/extra labels.                                                                                                                                                                                                                                                          |
|   5. | ***Ln***: 64<br><br>**`RandomForestClassifier()` has no seed/config**                | Non-reproducible model artifacts.                                                                                                  | Set `random_state`; store config/code/data versions.                                                   | Deterministic training test.<br><br>**`Refs`**: [*scikit-learn.org/RandomForestClassifier*](https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestClassifier.html)                                                                                              |
|   6. | ***Ln***: 64-70<br><br>**No model artifact governance**                              | No model version, registry, approval, rollback, threshold record.                                                                  | Store model version, feature schema, metrics, threshold, data snapshot.                                | Release gate requires model card/approval.                                                                                                                                                                                                                                               |
|   7. | ***Ln***: 73-76<br><br>**No event schema validation at scoring**                     | Bad live events can crash or mis-score.                                                                                            | Validate required fields & types before scoring.                                                       | **`Tests`**: missing `account_id`, malformed event.                                                                                                                                                                                                                                      |
|   8. | ***Ln***: 45-57<br><br>**Missing empty-history defaults**                            | Not reachable from current grouped training path, but will break cold-start accounts, feature-store rows, or future scoring paths. | Define explicit empty-history defaults.                                                                | Unit test empty event list/account.<br><br>Keep this lower priority than the existing unseen-account scoring crash.                                                                                                                                                                      |
...
| Sl.# | Issue & Ln #                                                                                | Blocker (*Tl;Dr*)                                                                                               | Mitigate/Resolve                                                        | Notes/tests/long-term fixes                                                                                                                       |
| ---: | ------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
|   1. | ***Ln***: 13, 63<br><br>**Standard logger only, no structured logging**                     | Poor prod diagnostics & weak log governance.                                                                    | Use structured logger with redaction & standard fields.                 | Include trace/event/tenant IDs.<br><br>Verify no secrets.                                                                                         |
|   2. | ***Ln***: 54-55<br><br>**Comment identifies throughput issue but implementation is unsafe** | Good intent, poor concurrency model.                                                                            | Replace goroutine-per-event with partition-aware bounded worker pool.   | Keep goal: slow account should not block unrelated accounts.                                                                                      |
|   3. | ***Ln***: 103<br><br>**`json.Marshal` errors are ignored**                                  | In this exact payload shape, failure is unlikely, but event construction should still be checked before append. | Check marshal errors before append.<br><br>Prefer typed event encoders. | Add error-handling coverage for event construction paths.<br><br>**`Refs`**: [*go.dev/error-handling*](https://go.dev/blog/error-handling-and-go) |
...
### Primary code blockers

1. Credential handling is unsafe.

2. Immutable event log can be polluted with secrets.

3. Failed events can be acknowledged & lost.

4. At-least-once delivery is not handled idempotently.

5. Same-account ordering is not preserved.

6. Projection writes are not concurrency-safe.

7. Event schemas are not validated/versioned.

8. Risk model evaluation is invalid.

9. Accuracy is unsuitable for rare compromise detection.
...
1. Supply-chain, SBOM, package-version, CVE, & license controls.

2. CI coverage gates across unit, validation, regression, & integration tests.

3. Severity telemetry model.

4. Structured observability.

5. Request, tenant, & header contracts.

6. Rate limiting, throttling, backoff, & Retry-After / X-RateLimit handling.
